...
- **DO runtime** ([`proactive-agent.ts`](src/agent/proactive-agent.ts)): a plain `Agent<Env>` exposing one public RPC method, `converse(text, identity)`; `this.sql` backs Sessions. The Worker owns the single A2A JSON-RPC server ([`index.ts`](src/index.ts) + [`executor.ts`](src/a2a/executor.ts)) and reaches the DO with a native Cloudflare RPC call (`stub.converse(...)`), passing the **verified** identity as a typed argument — no internal HTTP/JSON-RPC layer on the DO (it's a private implementation detail of the Worker, not a network-reachable service).
