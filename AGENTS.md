...
| [src/env.ts](src/env.ts)                                     | `Env` interface (bindings + secrets): `A2A_SIGNING_KEY`, `GATEWAY_ORIGINS`, `AI`, `ProactiveAgent`.                                                                                                         |
...
| [src/agent/config.ts](src/agent/config.ts)                   | Model ids, AI Gateway slug, loop bound, and Session/compaction tuning (constants).                                                                                                                          |
...
- Module entry is `satisfies ExportedHandler<Env>` and re-exports the `ProactiveAgent` DO class (Cloudflare resolves the `class_name` from the entry exports); bindings are typed via the `Env` interface in [src/env.ts](src/env.ts) (the `AI` binding and `ProactiveAgent` DO are mirrored by `wrangler.jsonc`; secrets are not).
