...
| [wrangler.jsonc](wrangler.jsonc)                                   | Worker config: `AI` + `VECTORIZE` bindings + `ProactiveAgent` Durable Object (SQLite migration). Secrets live outside it.                                                                                   |
...
- Module entry is `satisfies ExportedHandler<Env>` and re-exports the `ProactiveAgent` DO class (Cloudflare resolves the `class_name` from the entry exports); bindings and secrets are typed via the `Env` interface in [worker-configuration.d.ts](worker-configuration.d.ts) (generated; includes `AI`, `VECTORIZE`, `ProactiveAgent`, `A2A_SIGNING_KEY`, `GATEWAY_ORIGINS`).
